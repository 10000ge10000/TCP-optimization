...
- HTTP Agent 在获取并发槽之前完成鉴权与请求体读取，慢速客户端不再占用并发额度。
- `/test` 改为把校验阶段解析出的 IP 直接交给 iperf3，消除校验与连接之间的 DNS 重绑定窗口。

### Fixed

- `profile_values` 对未知预设显式返回非零，调用方在关闭 `set -e` 的上下文中也不会拿到空值。

### Tests

- 新增 Agent 慢速请求体不占槽、测试目标地址钉住两项集成用例。
- 新增预制参数别名等价、字段数、挡位单调性与未知预设拒绝断言，以及菜单失败守卫回归检查。
